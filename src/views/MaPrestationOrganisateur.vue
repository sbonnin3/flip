  ...
  <div class="prestation-createur">
    ...
    <form @submit.prevent="saveStand">
      ...
      <div class="form-group">
        ...
        <select v-model="stand.idPoint" id="point" @change="handlePointSelection" required>
...
export default {
  ...
  computed: {
    ...
    availablePoints() {
      return points.filter(
        point => point.category === "Emplacement" && point.disponible
      );
    }
  },
  created() {
  ...
  methods: {
    ...
    handlePointSelection() {
      // Si un point était précédemment sélectionné, le libérer
      if (this.originalPointId) {
        this.updatePointAvailability(this.originalPointId, true);
      }

      // Marquer le nouveau point comme temporairement indisponible
    ...
    saveStand() {
      ...
      // Marquer le point comme occupé définitivement
      this.updatePointAvailability(this.stand.idPoint, false);
  ...
  beforeDestroy() {
    // Libérer le point lorsque le prestataire quitte la page sans enregistrer
